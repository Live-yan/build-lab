...
from NetSDK.SDK_Callback import fDisConnect, fHaveReConnect,LOG_SET_PRINT_INFO
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def _init_ui(self):
        ...
        self.IP_lineEdit.setText('172.13.137.176')
    ...
    def login_btn_onclick(self):
        if not self.loginID:
            ...
            if self.loginID != 0:
                ...
                self.downloadType_comboBox.setEnabled(True)
    ...
    def download_btn_onclick(self):
        if not self.downloadID:
            ...
            nchannel = self.Channel_comboBox.currentIndex()
            if self.downloadType_comboBox.currentIndex() == 0:
                ...
                stuInParam = NET_IN_DOWNLOAD_BY_DATA_TYPE()
                stuInParam.dwSize = sizeof(NET_IN_DOWNLOAD_BY_DATA_TYPE)
                stuInParam.cbDownLoadPos = TimeDownLoadPosCallBack
                stuInParam.cbRealData = DownLoadDataCallBack
                stuInParam.nChannelID = nchannel
                stuInParam.emDataType = self.downloadType_comboBox.currentIndex()-1
                print(stuInParam.emDataType)
                stuInParam.stStartTime.dwYear =start_date.year()
                stuInParam.stStartTime.dwMonth = start_date.month()
                stuInParam.stStartTime.dwDay = start_date.day()
                stuInParam.stStartTime.dwHour = start_time.hour()
                stuInParam.stStartTime.dwMinute = start_time.minute()
                stuInParam.stStartTime.dwSecond = start_time.second()

                stuInParam.stStopTime.dwYear = end_date.year()
                stuInParam.stStopTime.dwMonth = end_date.month()
                stuInParam.stStopTime.dwDay = end_date.day()
                stuInParam.stStopTime.dwHour = end_time.hour()
                stuInParam.stStopTime.dwMinute = end_time.minute()
                stuInParam.stStopTime.dwSecond = end_time.second()

                #input_value = input(save_file_name)
                OldPath = save_file_name.encode('gbk')
                stuInParam.szSavedFileName = c_char_p(OldPath)
                print(stuInParam.szSavedFileName)

                stuOutParam = NET_OUT_DOWNLOAD_BY_DATA_TYPE()
                stuOutParam.dwSize = sizeof(NET_OUT_DOWNLOAD_BY_DATA_TYPE)

                self.downloadID = self.sdk.DownloadByDataType(self.loginID, stuInParam, stuOutParam, 5000)
                if self.downloadID:
                    self.Download_pushButton.setText("停止(Stop)")
                else:
                    QMessageBox.about(self, '提示(prompt)', self.sdk.GetLastErrorMessage())
    ...
    def query_file(self, startTime, endTime):
        ...
        if nchannel < 0:
            nchannel = 0
        result, fileCount, infos = self.sdk.QueryRecordFile(self.loginID, nchannel, int(EM_QUERY_RECORD_TYPE.ALL), startTime,
